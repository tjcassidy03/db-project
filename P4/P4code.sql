...
CREATE FUNCTION Spotify_Project.PlaylistLength (@playlist_id INT)
RETURNS INT
AS
BEGIN
...
CREATE FUNCTION Spotify_Project.SongCount (@artist_id INT)
RETURNS INT
AS
BEGIN
  DECLARE @count INT;
...
CREATE FUNCTION Spotify_Project.SongCount (@album_id INT)
RETURNS INT
AS
BEGIN
  DECLARE @total_dur INT;
  SELECT @total_dur = SUM(duration_ms)
  FROM (
    SELECT * 
    FROM album_songs
    INNER JOIN Song ON album_songs.track_id = Song.track_id
